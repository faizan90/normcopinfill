...
class FlagSusp:
    ...
    def plot(self, args):
        ...
        interp_data_idxs = logical_or(
            isnan(act_var),
            isnan(out_conf_df[
                self.fin_conf_head].loc[self.infill_dates].values))
        ...
        conf_var_idxs_lo = where(
            not_interp_data_idxs, act_var_lo < conf_var_vals_lo, False)

        conf_var_idxs_hi = where(
            not_interp_data_idxs, act_var_hi > conf_var_vals_hi, False)

        conf_var_idxs_wi_1 = where(
            not_interp_data_idxs, act_var_hi >= conf_var_vals_lo, False)

        conf_var_idxs_wi_2 = where(
            not_interp_data_idxs, act_var_lo <= conf_var_vals_hi, False)

        conf_var_idxs_wi = logical_and(conf_var_idxs_wi_1, conf_var_idxs_wi_2)
        ...
        if not self.update_summary_df_only:
            ...
            _y_ticks = ['below_%0.3fP' % self.flag_probs[0],
                        'within\n%0.3fP_&_%0.3fP' % (
                            self.flag_probs[0], self.flag_probs[1]),
