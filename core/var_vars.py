...
class DiscContVars:
    ...
    def get_disc_vars(
        ...
        if gy_arr.shape[0] <= 1:
            gy_val = norm_cdf_py(divide(mu_t, sig_sq_t ** 0.5))
            interp_val = interp(
                gy_val,
                val_cdf_ftn.y,
                val_cdf_ftn.x,
                left=val_cdf_ftn.x.min(),
                right=val_cdf_ftn.x.max())

            if False:
                print('\n\nval_arr_adj.shape[0] <= 1 on:', infill_date)
                print('curr_infill_stn:', self.curr_infill_stn)
                print('mu_t:', mu_t)
                print('sig_sq_t:', sig_sq_t)
                print('gy_val:', gy_val)
                print('interp_val:', interp_val)
                print(
                    'Nebor series:\n',
                    self.in_var_df.loc[infill_date].dropna(),
                    end='')
                print('\n\n')

            fin_val_ppf_ftn_adj = interp1d(
                linspace(0, 1.0, 10),
                [interp_val] * 10,
                bounds_error=False,
                fill_value=(interp_val, interp_val))

            fin_val_grad_ftn_adj = interp1d(
                (curr_min_var_val, curr_max_var_val),
                (0, 0),
                bounds_error=False,
                fill_value=(0, 0))
            ...
            if curr_max_var_val_adj > self.var_ge_trs:
                adj_val_idxs = logical_and(
                    val_arr >= self.var_ge_trs, val_arr <= curr_max_var_val_adj)
                val_arr_adj = val_arr[adj_val_idxs]

                if 1 < val_arr_adj.shape[0] < self.n_discret:
            ...
            for i, val_adj in enumerate(val_arr_adj):
                if val_adj > self.var_ge_trs:
                    ...
                    values_arr = (
                        self.in_var_df.loc[
                            infill_date,
                            curr_var_df.columns[1:]].dropna().values)
            ...
            adj_probs_idxs = logical_and(
                adj_probs_idxs, gy_arr_adj <= self.adj_prob_bounds[1])
            ...
            fin_val_grad_ftn_adj = interp1d(
                val_arr_adj, pdf_arr_adj, bounds_error=False, fill_value=(0, 0))
    ...
    def get_cont_vars(
        ...
        if val_arr_adj.shape[0] <= 1:
            ...
            fin_val_grad_ftn = interp1d(
                (val_arr.min(), val_arr.max()),
                (0, 0),
                bounds_error=False,
                fill_value=(0, 0))
            ...
            (curr_min_var_val_adj,
             curr_max_var_val_adj) = (val_arr_adj.min(), val_arr_adj.max())
