...
class ECops:
        ...
        if self.nrst_stns_type == 'dist':
            ...
            if not norm_cop_obj._rank_corr_cmptd:
                if self.max_time_lag_corr:
                    BestLagRankCorrStns(norm_cop_obj)
                else:
    ...
    def _plot_ecop(self,
        ...
        if self.max_time_lag_corr:
            curr_lag_dict = self.time_lags_dict[infill_stn]
    ...
    def __plot_ecops(self, infill_stns):
        ...
        if self.infill_type == 'discharge':
            ...
            assert False, as_err('Incorrect \'infill_type\': %s!' %
                                 str(self.infill_type))
